...
describe("metrics — tool call outcomes", () => {
  ...
  it("records auth_failed + scope_denied for an unknown buyer (no token)", async () => {
  ...
  it("records replay_rejected on a duplicated client_request_id", async () => {
    await ctx.client.callTool({
      name: "discover_products",
      arguments: { buyer_id: ENTITLED_BUYER, client_request_id: "dup-metric-1" },
    });
    await ctx.client.callTool({
      name: "discover_products",
      arguments: { buyer_id: ENTITLED_BUYER, client_request_id: "dup-metric-1" },
...
describe("metrics — exposition format and security invariant", () => {
  ...
  it("NEVER leaks a buyer_id into any rendered label (SEC invariant)", async () => {
    ...
    // Drive every outcome class with a real buyer id present in the arguments.
    await ctx.client.callTool({ name: "discover_products", arguments: { buyer_id: ENTITLED_BUYER } });
    await ctx.client.callTool({ name: "discover_products", arguments: { buyer_id: UNKNOWN_BUYER } });
    await ctx.client.callTool({
      name: "get_forecast",
